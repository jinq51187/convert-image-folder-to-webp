...
# Folder selection
Label(root, text="Folder:", anchor="w").pack(fill="x", padx=10, pady=(10,0))
...
Checkbutton(options_frame, text="Recursive (include subfolders)", variable=recursive_var).pack(anchor="w")
Checkbutton(options_frame, text="Delete original after convert", variable=delete_var).pack(anchor="w")

Label(options_frame, text="Quality (0-100):").pack(anchor="w", pady=(10,0))
Entry(options_frame, textvariable=quality_var, width=5).pack(anchor="w")

Label(options_frame, text="Save mode:").pack(anchor="w", pady=(10,0))
Radiobutton(options_frame, text="Overwrite existing .webp", variable=save_mode, value=1).pack(anchor="w")
Radiobutton(options_frame, text="Create new (_new.webp)", variable=save_mode, value=2).pack(anchor="w")

# Convert button
Button(root, text="Convert", bg="#4CAF50", fg="white", font=("Arial", 12, "bold"), command=convert_images).pack(pady=10)
